var _carbonads = {
    ...
    getServe: function(serve, placement) {
        ...
        if (this.isset(r[placement]) && (serve == 'CVYD42T' || serve == 'CVYD42E' || serve == 'C6AILKT')) return r[placement];
...
function _carbonads_go(b) {
    ...
    if (typeof ad.custom_css != 'undefined') {
        ...
        document.getElementById('_carbonads_js').parentNode.insertBefore(css, document.getElementById('_carbonads_js'))
    }
    if (ad.html != null) {
    ...
    if (typeof link[1] != 'undefined' && _carbonads.isset(ad.statlink)) fulllink = link[0] + '?segment=placement:' + placement + ';&encredirect=' + encodeURIComponent(link[1]);
    else if (link[0].search('srv.buysellads.com') > 0 && _carbonads.isset(ad.statlink)) fulllink = link[0] + '?segment=placement:' + placement + ';';
    else if (Array.isArray(link)) fulllink = link[0];
    else fulllink = link;
    fulllink = fulllink.replace(/srv.buysellads.com/g, 'srv.carbonads.net');
    var el = document.createElement('span');
    el.innerHTML = '<span class="carbon-wrap"><a href="https:' + fulllink.replace('[timestamp]', time) + '" class="carbon-img" target="_blank" rel="noopener sponsored"><img src="' + image + '" alt="ads via Carbon" border="0"' + (_carbonads.isset(bgcolor) ? 'style="background:' + bgcolor + ';width:100px;padding: 30px 15px;box-sizing: content-box;" ' : 'height="100" width="130"') + ' /></a><a href="https:' + fulllink.replace('[timestamp]', time) + '" class="carbon-text" target="_blank" rel="noopener sponsored">' + _carbonads.htmlEncode(description) + '</a></span>';
    if (!_carbonads.isset(ad.removecarbon)) el.innerHTML += '<a href="http://carbonads.net/?utm_source=' + placement + '&utm_medium=ad_via_link&utm_campaign=in_unit&utm_term=carbon" class="carbon-poweredby" target="_blank" rel="noopener sponsored">ads via Carbon</a>';
